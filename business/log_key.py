...
from device.dpi_constants import (
    ...
    access_log_rulefile, xsa_jsonfile,
    fz_block_rulefile, fz_block_add_rulefile, fz_block_del_rulefile,
    ip_block_rulefile, ip_block_add_rulefile, ip_block_del_rulefile,
    fz_monitor_rulefile, fz_monitor_add_rulefile, fz_monitor_del_rulefile,
    ip_monitor_rulefile, ip_monitor_add_rulefile, ip_monitor_del_rulefile,
    fz_blurmatch_rulefile, fz_blurmatch_add_rulefile,
    fz_action_txtfile, ip_segment_txtfile, fz_template_txtfile,
...
def log_key(p_excel: dict, sheets, path="用例", newpath=None,
    ...
    try:
        for sheet_name in sheets:
            ...
            # DPI环境初始化
            active_case_count = sum(1 for x in cases.values() if x[0].get("执行状态") and int(x[0]["执行状态"]) == 1)
            logger.info(f"活跃用例数：{active_case_count}，总用例数：{len(cases)}")
            ...
            for case_name, case in cases.items():
                ...
                if case[0].get("策略", None):
                    ...
                    policys += policy
            logger.info(b"\n".join(policys).decode("utf-8") if policys else "marex策略：No policies")
            ...
            # gwfz_log 用例类型处理：根据页签列头动态收集并加载规则文件
            if config.get(f"{sheet_name}_casetype", None) == "gwfz_log":
                logger.info("gwfz_log 用例类型，收集反诈策略")

                # 规则文件列名 → (远程路径, 替换方式)
                # "marex" = 行首用 1+commandId[-6:], id= 用 commandId; "raw" = 行首替换为 commandId, 其余原样
                rule_columns = {
                    "fz_block.rule": (fz_block_rulefile, "commandId"),
                    "fz_block_add.rule": (fz_block_add_rulefile, "commandId"),
                    "fz_block_del.rule": (fz_block_del_rulefile, "commandId"),
                    "ip_block.rule": (ip_block_rulefile, "commandId"),
                    "ip_block_add.rule": (ip_block_add_rulefile, "commandId"),
                    "ip_block_del.rule": (ip_block_del_rulefile, "commandId"),
                    "fz_monitor.rule": (fz_monitor_rulefile, "commandId"),
                    "fz_monitor_add.rule": (fz_monitor_add_rulefile, "commandId"),
                    "fz_monitor_del.rule": (fz_monitor_del_rulefile, "commandId"),
                    "ip_monitor.rule": (ip_monitor_rulefile, "commandId"),
                    "ip_monitor_add.rule": (ip_monitor_add_rulefile, "commandId"),
                    "ip_monitor_del.rule": (ip_monitor_del_rulefile, "commandId"),
                    "fz_blurmatch.rule": (fz_blurmatch_rulefile, "marex"),
                    "fz_blurmatch_add.rule": (fz_blurmatch_add_rulefile, "marex"),
                    "fz_action.txt": (fz_action_txtfile, "plain"),
                    "ip_segment.txt": (ip_segment_txtfile, "plain"),
                    "fz_template.txt": (fz_template_txtfile, "plain"),
                }

                # 从页签列头中筛选出存在的规则文件列
                sheet_heads = p_excel["sheet_name2heads"][sheet_name]
                active_rules = {col: info for col, info in rule_columns.items() if col in sheet_heads}

                if not active_rules:
                    logger.info("未找到规则文件列，跳过策略加载")
                else:
                    logger.info(f"规则文件列：{list(active_rules.keys())}")

                    # 收集策略
                    policys = {col: list() for col in active_rules}
                    for case_name, case in cases.items():
                        if not case[0].get("用例名") or str(case[0].get("执行状态")) != "1":
                            continue
                        for col, (remotepath, replace_mode) in active_rules.items():
                            if case[0].get(col):
                                rules = [r for r in case[0][col].strip().split("\n") if r.strip()]
                                if replace_mode == "commandId":
                                    policys[col] += [re.sub(r"^\d+", case[0]['commandId'], r).encode("utf-8") for r in rules]
                                elif replace_mode == "marex":
                                    policys[col] += [re.sub(r"^\d+", '1' + case[0]['commandId'][-6:], re.sub(r"(?<=id=)\d+", case[0]['commandId'], r)).encode("utf-8") for r in rules]
                                else:  # plain: 原样加载
                                    policys[col] += [r.encode("utf-8") for r in rules]

                    # 去重、打印、加载
                    for col, (remotepath, _) in active_rules.items():
                        policys[col] = list(dict.fromkeys(policys[col]))
                        content = b"\n".join(policys[col]).decode("utf-8") if policys[col] else "空"
                        logger.info(f"{col}({len(policys[col])}条)：{content}")
                        dpi_xsa.marex_policy_update(policy=policys[col], path=remotepath, md5check=True)
            ...
            elif pcaps_tmp and config.get(sheet_name + "_sendpktsmode") != "scapy_send":
                raise RuntimeError(f"请检查发包模式配置，sendpktsmode={config.get(sheet_name + '_sendpktsmode')}")
            else:
                logger.info("无需发包，跳过")
            ...
            if not config.get(f"{sheet_name}_reportrule", None) and (
                ...
                # 区分XML和普通文本的处理方式
                fileparsertype = config.get(f"{sheet_name}_fileparsertype", "")
                logger.info(f"fileparsertype: [{fileparsertype}]")
                if config.get(sheet_name + "_filetype") == "xml" and not config.get(sheet_name + "_splitflag"):
                    ...
                    for name in response:
                        ...
                        for record in records:
                            ...
                            key = "_".join([str(fields[i]) if fields[i] is not None else "" for i in keyvalue_headNos]).lower() if keyvalue_headNos else "log"
                            if key in act_log_dict:
                    ...
                    for line in log_lines:
                        ...
                        key = "_".join([str(fields[x]) if fields[x] is not None else "" for x in keyvalue_headNos]).lower() if keyvalue_headNos else "log"
                        if key in act_log_dict:
                            act_log_dict[key].append(fields)
                        else:
                            act_log_dict[key] = [fields]
            ...
            for case_name, case in cases.items():
                ...
                mark = list()
                key = "_".join([str(case[0].get(x, "")) if case[0].get(x, "") is not None else "" for x in keynames]).lower() if keynames else "log"

...
```mermaid
graph LR
    Start([版本号字符串]) --> A[分割版本号<br/>按 . 和 - 分割]
    A --> B{部分类型判断}
    
    B -->|数字| C[转换为整数]
    B -->|alpha/beta/rc/patch| D[转换为权重<br/>alpha=-3, beta=-2<br/>rc=-1, patch=1]
    B -->|其他| E[转换为 0]
...
```mermaid
graph TD
    Start([用例配置为<br/>target_version]) --> A[解析模式参数]
    A --> B[根据模式获取分类]
    B --> C{配置中是否有<br/>target_version_分类?}
    
    C -->|是| D[读取配置值]
    C -->|否| E{该分类是否已<br/>刷新过 RDM?}
    
    E -->|是| F[直接从 versions.json<br/>读取所有版本]
    E -->|否| G[从 RDM 平台<br/>刷新 versions.json]
